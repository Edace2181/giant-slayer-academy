(function () {
  ...
  function render(bank) {
    ...
    kingdomProgress.textContent = `${completed} of ${bank.missions.length} Batch 5 missions complete`;
  ...
  fetch(BANK_URL)
    ...
    .then(bank => {
      if (!Array.isArray(bank.missions) || bank.missions.length !== 25) {
        throw new Error("Linux Labs Batch 5 must contain exactly twenty-five missions.");
      }
      render(bank);
    })
    .catch(problem => {
