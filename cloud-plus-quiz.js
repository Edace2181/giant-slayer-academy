...
const objectiveFiles = [
  "json/cloud-plus/world1/1.1-hatchling.json",
  "json/cloud-plus/world1/1.2-hatchling.json",
  "json/cloud-plus/world1/1.3-hatchling.json",
  "json/cloud-plus/world1/1.4-hatchling.json",
  "json/cloud-plus/world1/1.5-hatchling.json",
  "json/cloud-plus/world1/1.6-hatchling.json",
  "json/cloud-plus/world1/1.7-hatchling.json",
  "json/cloud-plus/world1/1.8-hatchling.json",
  "json/cloud-plus/world1/1.9-hatchling.json",
  "json/cloud-plus/world1/1.10-hatchling.json",
  "json/cloud-plus/world1/1.11-hatchling.json",
  "json/cloud-plus/world2/2.1-hatchling.json",
  "json/cloud-plus/world2/2.2-hatchling.json",
  "json/cloud-plus/world2/2.3-hatchling.json",
  "json/cloud-plus/world2/2.4-hatchling.json",
  "json/cloud-plus/world2/2.5-hatchling.json",
  "json/cloud-plus/world3/3.1-hatchling.json",
  "json/cloud-plus/world3/3.2-hatchling.json",
  "json/cloud-plus/world3/3.3-hatchling.json",
  "json/cloud-plus/world3/3.4-hatchling.json",
  "json/cloud-plus/world4/4.1-hatchling.json",
  "json/cloud-plus/world4/4.2-hatchling.json",
  "json/cloud-plus/world4/4.3-hatchling.json",
  "json/cloud-plus/world4/4.4-hatchling.json",
  "json/cloud-plus/world4/4.5-hatchling.json",
  "json/cloud-plus/world4/4.6-hatchling.json",
  "json/cloud-plus/world5/5.1-hatchling.json",
  "json/cloud-plus/world5/5.2-hatchling.json",
  "json/cloud-plus/world5/5.3-hatchling.json",
  "json/cloud-plus/world5/5.4-hatchling.json",
  "json/cloud-plus/world5/6.1-hatchling.json",
  "json/cloud-plus/world5/6.2-hatchling.json",
  "json/cloud-plus/world5/6.3-hatchling.json"
];
...
const captainModes = {
  "boss-rush-1": { name: "Boss Rush I", file: "json/cloud-plus/captains/boss-rush-1.json" },
  "boss-rush-2": { name: "Boss Rush II", file: "json/cloud-plus/captains/boss-rush-2.json" },
  "weakness-captains": { name: "Weakness Captain Rush", file: "json/cloud-plus/captains/weakness-captains.json" },
  "final-captain-rush": { name: "Final Captain Rush", file: "json/cloud-plus/captains/final-captain-rush.json" }
};
...
function buildWeaknessMix(allQuestions, count) {
  const config = window.HydraFlags?.configFor("cloud-plus");
...
function resultDestination() {
  if (exam) return { href: "cloud-plus-final-gauntlet.html", label: "Return to Cloud+ Final Boss" };
  if (world === "7") return { href: "cloud-plus-world7.html", label: "Return to Chapter 7" };
  if (world === "6") return { href: "cloud-plus-world6.html", label: "Return to Chapter 6" };
  return { href: `cloud-plus-world${world}-objectives.html`, label: `Return to Chapter ${world}` };
}

function showPlaceholder() {
  questionCount.textContent = "Content Pending";
  questionEl.textContent = "This Cloud+ question bank is ready for content.";
  answersEl.replaceChildren();
  feedback.textContent = "Cloud+ skeleton validated. Questions will be added during the appropriate production phase.";
...
async function loadQuiz() {
  try {
    ...
    if (exam) {
      questions = await fetchBank(`json/cloud-plus/final-dungeon/practice-exam-${exam}.json`);
      quizInfo.textContent = `🏰 Cloud+ • Practice Exam ${exam}`;
      ...
      quizInfo.textContent = `🌋 Cloud+ Chapter 7 • ${captainModes[mode].name}`;
      ...
      const weaknessMix = mode === "weakness-mix" ? buildWeaknessMix(unique, world6Modes[mode].count) : null;
      questions = weaknessMix ? weaknessMix.questions : shuffle(unique).slice(0, world6Modes[mode].count);
      quizInfo.textContent = `❄️ Cloud+ Chapter 6 • ${world6Modes[mode].name}`;
      ...
      questions = await fetchBank(`json/cloud-plus/world${world}/${objective}-hatchling.json`);
      quizInfo.textContent = `Cloud+ Chapter ${world} • Objective ${objective}`;
      objectiveTitle.textContent = `Objective ${objective}`;
      objectiveName.textContent = questions[0]?.blueprint || "";
    } else {
      throw new Error("Unknown or incomplete Cloud+ quiz mode.");
    }
...
function showQuestion() {
  ...
  window.HydraFlags?.setCurrentQuestion({
    ...
    world,
    objective,
    bankPath: `json/cloud-plus/world${world}/${objective}-hatchling.json`
  });
}
...
function showResults(options = {}) {
  ...
  if (world === "7" && captainModes[mode]) {
    window.HydraCampaignUI?.saveQuizResult({ type: "captain", id: mode, percent, passed, label: captainModes[mode].name, href: `cloud-plus-quiz.html?world=7&mode=${mode}` });
  } else if (exam) {
    window.HydraCampaignUI?.saveQuizResult({ type: "exam", id: exam, percent, passed, label: `Practice Exam ${exam}`, href: `cloud-plus-quiz.html?exam=${exam}` });
  }
  window.HydraCampaignUI?.recordSession({
    ...
    label: objective && /^[1-5]$/.test(world) ? `Objective ${objective}` : world === "7" && captainModes[mode] ? captainModes[mode].name : exam ? `Practice Exam ${exam}` : mode || `Chapter ${world} Review`,
    correct: score,
