(function () {
  ...
  const elements = {
    loading: document.getElementById("linuxManualLoading"), error: document.getElementById("linuxManualError"),
    errorMessage: document.getElementById("linuxManualErrorMessage"), errorReturn: document.getElementById("linuxManualErrorReturn"),
    application: document.getElementById("linuxManualApplication"), kicker: document.getElementById("linuxManualKicker"),
    objective: document.getElementById("linuxManualObjective"), completionNote: document.getElementById("linuxManualCompletionNote"),
    navigation: document.getElementById("linuxManualNavigation"), navigationLinks: document.getElementById("linuxManualNavigationLinks"),
    sections: document.getElementById("linuxManualSections"), miniForm: document.getElementById("linuxMiniCheckForm"),
    miniPrompt: document.getElementById("linuxMiniCheckPrompt"), miniChoices: document.getElementById("linuxMiniCheckChoices"),
    miniFeedback: document.getElementById("linuxMiniCheckFeedback"), complete: document.getElementById("linuxManualComplete"),
    returnLink: document.getElementById("linuxManualReturn")
  };
  let lesson = null;
  let miniCheck = null;
  ...
  function validateLesson(candidate) {
    ...
    if (candidate.certification !== "linux-essentials" || candidate.examCode !== "010-160" || candidate.blueprintVersion !== "1.6") throw new Error("The Field Manual does not match LPI Linux Essentials.");
  ...
  function renderNavigation() {
    ...
    miniCheckLink.href = "#linuxMiniCheckHeading"; elements.navigationLinks.append(miniCheckLink);
  ...
  function renderMiniCheck() {
    ...
    miniCheck.choices.forEach((choice, index) => {
      ...
      input.type = "radio"; input.name = "linux-mini-check-answer"; input.value = String(index);
  ...
  function showRouteError(message) {
    ...
    elements.errorReturn.href = world === "1" ? "linux-essentials-world1-objectives.html" : "linux-essentials-campaign.html";
    elements.errorReturn.textContent = world === "1" ? "← Return to World 1 Objective Hub" : "← Return to Linux Essentials Campaign Map";
  }

  elements.miniForm.addEventListener("submit", event => {
    event.preventDefault();
    const selected = elements.miniForm.querySelector('input[name="linux-mini-check-answer"]:checked');
    if (!selected) {
  ...
  elements.complete.addEventListener("click", () => {
    ...
    try {
      saveCompletion();
      window.location.assign("linux-essentials-quiz.html?world=" + encodeURIComponent(world) + "&objective=" + encodeURIComponent(objective));
    } catch (_) {
      elements.miniFeedback.className = "security-mini-check-feedback is-incorrect";
  ...
  async function initialize() {
    ...
    try {
      const lessonResponse = await fetch("json/linux-essentials/field-manual/" + objective + ".json");
      if (!lessonResponse.ok) throw new Error("This Linux Essentials Field Manual has not been published yet.");
      lesson = validateLesson(await lessonResponse.json());
      const bankResponse = await fetch("json/linux-essentials/world" + world + "/" + objective + "-hatchling.json");
      ...
      elements.kicker.textContent = "LPI Linux Essentials · 010-160 v1.6 · World " + world + " · Objective " + objective;
      elements.objective.textContent = lesson.title;
      elements.returnLink.href = "linux-essentials-world" + world + "-objectives.html";
