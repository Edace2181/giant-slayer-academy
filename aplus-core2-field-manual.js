(function () {
  ...
  const elements = {
    ...
    application: document.getElementById("aplusManualApplication"), kicker: document.getElementById("aplusManualKicker"),
    objective: document.getElementById("aplusManualObjective"), completionNote: document.getElementById("aplusManualCompletionNote"),
    navigation: document.getElementById("aplusManualNavigation"), navigationLinks: document.getElementById("aplusManualNavigationLinks"),
    sections: document.getElementById("aplusManualSections"), miniForm: document.getElementById("aplusMiniCheckForm"),
    miniPrompt: document.getElementById("aplusMiniCheckPrompt"), miniChoices: document.getElementById("aplusMiniCheckChoices"),
    miniFeedback: document.getElementById("aplusMiniCheckFeedback"), complete: document.getElementById("aplusManualComplete"),
  ...
  function validateLesson(candidate) {
    ...
    if (candidate.certification !== "aplus-core2" || candidate.examCode !== "220-1202") throw new Error("The Field Manual does not match A+ Core 2.");
  ...
  function renderNavigation() {
    ...
    miniCheckLink.href = "#aplusMiniCheckHeading"; elements.navigationLinks.append(miniCheckLink);
  ...
  function renderMiniCheck() {
    ...
    miniCheck.choices.forEach((choice, index) => {
      ...
      input.type = "radio"; input.name = "aplus-mini-check-answer"; input.value = String(index);
  ...
  function showRouteError(message) {
    ...
    elements.errorReturn.href = /^[1-5]$/.test(world) ? "aplus-core2-world" + world + "-objectives.html" : "aplus-core2-campaign.html";
    elements.errorReturn.textContent = /^[1-5]$/.test(world) ? "← Return to World " + world + " Objective Hub" : "← Return to A+ Core 2 Campaign Map";
  }
  ...
  elements.complete.addEventListener("click", () => {
    ...
    try {
      saveCompletion();
      window.location.assign("aplus-core2-quiz.html?world=" + encodeURIComponent(world) + "&objective=" + encodeURIComponent(objective));
    } catch (_) {
      elements.miniFeedback.className = "security-mini-check-feedback is-incorrect";
  ...
  async function initialize() {
    ...
    try {
      const lessonResponse = await fetch("json/aplus-core2/field-manual/" + objective + ".json");
      if (!lessonResponse.ok) throw new Error("This A+ Core 2 Field Manual has not been published yet.");
      lesson = validateLesson(await lessonResponse.json());
      const bankResponse = await fetch("json/aplus-core2/world" + world + "/" + objective + "-hatchling.json");
      ...
      elements.kicker.textContent = "CompTIA A+ Core 2 · 220-1202 · World " + world + " · Objective " + objective;
      elements.objective.textContent = lesson.title;
      elements.returnLink.href = "aplus-core2-world" + world + "-objectives.html";
