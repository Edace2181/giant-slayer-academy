(function () {
  ...
  const MANUAL_KEY = "hydra-aws-cloud-practitioner-field-manual-v1";
  const params = new URLSearchParams(window.location.search);
  const world = params.get("world") || "";
  const objective = params.get("objective") || "";
  const elements = {
    loading: document.getElementById("awsManualLoading"), error: document.getElementById("awsManualError"),
    errorMessage: document.getElementById("awsManualErrorMessage"), errorReturn: document.getElementById("awsManualErrorReturn"),
    application: document.getElementById("awsManualApplication"), kicker: document.getElementById("awsManualKicker"),
    objective: document.getElementById("awsManualObjective"), completionNote: document.getElementById("awsManualCompletionNote"),
    navigation: document.getElementById("awsManualNavigation"), navigationLinks: document.getElementById("awsManualNavigationLinks"),
    sections: document.getElementById("awsManualSections"), miniForm: document.getElementById("awsMiniCheckForm"),
    miniPrompt: document.getElementById("awsMiniCheckPrompt"), miniChoices: document.getElementById("awsMiniCheckChoices"),
    miniFeedback: document.getElementById("awsMiniCheckFeedback"), complete: document.getElementById("awsManualComplete"),
    returnLink: document.getElementById("awsManualReturn")
  };
  let lesson = null;
  let miniCheck = null;
  ...
  function validateLesson(candidate) {
    ...
    if (candidate.certification !== "aws-cloud-practitioner" || candidate.examCode !== "CLF-C02") throw new Error("The Field Manual does not match AWS Cloud Practitioner.");
  ...
  function renderNavigation() {
    ...
    miniCheckLink.href = "#awsMiniCheckHeading"; elements.navigationLinks.append(miniCheckLink);
  ...
  function renderMiniCheck() {
    ...
    miniCheck.choices.forEach((choice, index) => {
      ...
      input.type = "radio"; input.name = "aws-mini-check-answer"; input.value = String(index);
  ...
  function showRouteError(message) {
    ...
    elements.errorReturn.href = /^[1-2]$/.test(world) ? "aws-cloud-practitioner-world" + world + "-objectives.html" : "aws-cloud-practitioner-campaign.html";
    elements.errorReturn.textContent = /^[1-2]$/.test(world) ? "← Return to World " + world + " Objective Hub" : "← Return to AWS Cloud Practitioner Campaign Map";
  }

  elements.miniForm.addEventListener("submit", event => {
    event.preventDefault();
    const selected = elements.miniForm.querySelector('input[name="aws-mini-check-answer"]:checked');
    if (!selected) {
    ...
    elements.miniFeedback.textContent = correct ? "Correct. " + miniCheck.explanation : "Not quite. Review the task-statement recognition cues and try again.";
  ...
  elements.complete.addEventListener("click", () => {
    ...
    try {
      saveCompletion();
      window.location.assign("aws-cloud-practitioner-quiz.html?world=" + encodeURIComponent(world) + "&objective=" + encodeURIComponent(objective));
    } catch (_) {
      elements.miniFeedback.className = "security-mini-check-feedback is-incorrect";
  ...
  async function initialize() {
    ...
    try {
      const lessonResponse = await fetch("json/aws-cloud-practitioner/field-manual/" + objective + ".json");
      if (!lessonResponse.ok) throw new Error("This AWS Cloud Practitioner Field Manual has not been published yet.");
      lesson = validateLesson(await lessonResponse.json());
      const bankResponse = await fetch("json/aws-cloud-practitioner/world" + world + "/" + objective + "-hatchling.json");
      ...
      elements.kicker.textContent = "AWS Certified Cloud Practitioner · CLF-C02 · World " + world + " · Task Statement " + objective;
      elements.objective.textContent = lesson.title;
      elements.returnLink.href = "aws-cloud-practitioner-world" + world + "-objectives.html";
