...
const objectiveFiles = [
  "json/security-plus/world1/1.1-hatchling.json",
  "json/security-plus/world1/1.2-hatchling.json",
  "json/security-plus/world1/1.3-hatchling.json",
  "json/security-plus/world1/1.4-hatchling.json",
  "json/security-plus/world2/2.1-hatchling.json",
  "json/security-plus/world2/2.2-hatchling.json",
  "json/security-plus/world2/2.3-hatchling.json",
  "json/security-plus/world2/2.4-hatchling.json",
  "json/security-plus/world2/2.5-hatchling.json",
  "json/security-plus/world3/3.1-hatchling.json",
  "json/security-plus/world3/3.2-hatchling.json",
  "json/security-plus/world3/3.3-hatchling.json",
  "json/security-plus/world3/3.4-hatchling.json",
  "json/security-plus/world4/4.1-hatchling.json",
  "json/security-plus/world4/4.2-hatchling.json",
  "json/security-plus/world4/4.3-hatchling.json",
  "json/security-plus/world4/4.4-hatchling.json",
  "json/security-plus/world4/4.5-hatchling.json",
  "json/security-plus/world4/4.6-hatchling.json",
  "json/security-plus/world4/4.7-hatchling.json",
  "json/security-plus/world4/4.8-hatchling.json",
  "json/security-plus/world4/4.9-hatchling.json",
  "json/security-plus/world5/5.1-hatchling.json",
  "json/security-plus/world5/5.2-hatchling.json",
  "json/security-plus/world5/5.3-hatchling.json",
  "json/security-plus/world5/5.4-hatchling.json",
  "json/security-plus/world5/5.5-hatchling.json",
  "json/security-plus/world5/5.6-hatchling.json"
];
const world6Modes = {
...
const captainModes = {
  "boss-rush-1": { name: "Boss Rush I", file: "json/security-plus/captains/boss-rush-1.json" },
  "boss-rush-2": { name: "Boss Rush II", file: "json/security-plus/captains/boss-rush-2.json" },
  "weakness-captains": { name: "Weakness Captain Rush", file: "json/security-plus/captains/weakness-captains.json" },
  "final-captain-rush": { name: "Final Captain Rush", file: "json/security-plus/captains/final-captain-rush.json" }
};
...
function resultDestination() {
  if (exam) return { href: "security-plus-final-gauntlet.html", label: "Return to Security+ Final Boss" };
  if (world === "7") return { href: "security-plus-world7.html", label: "Return to World 7" };
  if (world === "6") return { href: "security-plus-world6.html", label: "Return to World 6" };
  return { href: `security-plus-world${world}-objectives.html`, label: `Return to World ${world}` };
}
async function loadQuiz() {
  try {
    ...
    if (exam) {
      questions = await fetchBank(`json/security-plus/final-dungeon/practice-exam-${exam}.json`);
      quizInfo.textContent = `🏰 Security+ • Practice Exam ${exam}`;
      ...
      quizInfo.textContent = `🌋 Security+ World 7 • ${captainModes[mode].name}`;
      objectiveTitle.textContent = captainModes[mode].name;
      modeDisplay.textContent = captainModes[mode].name;
      returnLink.href = "security-plus-world7.html";
      ...
      const weaknessMix = mode === "weakness-mix"
        ? buildWeaknessMix(unique, world6Modes[mode].count, "security-plus")
        : null;
      questions = weaknessMix ? weaknessMix.questions : shuffle(unique).slice(0, world6Modes[mode].count);
      quizInfo.textContent = `❄️ Security+ World 6 • ${world6Modes[mode].name}`;
      ...
      questions = await fetchBank(`json/security-plus/world${world}/${objective}-hatchling.json`);
      quizInfo.textContent = `Security+ World ${world} • Objective ${objective}`;
      objectiveTitle.textContent = `Objective ${objective}`;
      objectiveName.textContent = questions[0]?.blueprint || "";
    } else {
      throw new Error("Unknown or incomplete Security+ quiz mode.");
    }
...
function showQuestion() {
  ...
  window.HydraFlags?.setCurrentQuestion({ question: q, world, objective, bankPath: `json/security-plus/world${world}/${objective}-hatchling.json` });
}
...
submitBtn.addEventListener("click", () => {
  ...
  if (/^[1-5]$/.test(world) && objective) {
    const reviewMetadata = { questionId: q.id, world, objective, bankPath: `json/security-plus/world${world}/${objective}-hatchling.json` };
...
function showResults(options = {}) {
  ...
  if (world === "7" && captainModes[mode]) {
    window.HydraCampaignUI?.saveQuizResult({ type: "captain", id: mode, percent, passed, label: captainModes[mode].name, href: `security-plus-quiz.html?world=7&mode=${mode}` });
  } else if (exam) {
    window.HydraCampaignUI?.saveQuizResult({ type: "exam", id: exam, percent, passed, label: `Practice Exam ${exam}`, href: `security-plus-quiz.html?exam=${exam}` });
  }
